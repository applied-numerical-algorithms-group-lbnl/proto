// 'euler_step' code generated by 'edavis' at 08/06/2019 09:49:44
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <math.h>
...
#define W_bar(c,z,y,x) array1[offset4((c),(z)+4,(y)+4,(x)+4,(N+3+4+1),(N+3+4+1),(N+3+4+1))]
#define u(c,z,y,x) array2[(c)]
#define W(c,z,y,x) array3[offset4((c),(z)+3,(y)+3,(x)+3,(N+2+3+1),(N+2+3+1),(N+2+3+1))]
#define umax(z,y,x) array0[0]
#define W_ave(c,z,y,x) array0[offset4((c),(z)+3,(y)+3,(x)+3,(N+2+3+1),(N+2+3+1),(N+2+3+1))]
...
#define W_aveL_d3(c,z,y,x) array5[offset4((c),(z),(y)+3,(x)+3,(N+1+1),(N+2+3+1),(N+2+3+1))]
#define W_aveH_d3(c,z,y,x) array4[offset4((c),(z)+1,(y)+3,(x)+3,(N+1+1),(N+2+3+1),(N+2+3+1))]
#define W_ave_f_d1(c,z,y,x) array0[offset4((c),(z)+3,(y)+3,(x),(N+2+3+1),(N+2+3+1),(N+1))]
#define F_bar_f_d1(c,z,y,x) array1[offset4((c),(z)+3,(y)+3,(x),(N+2+3+1),(N+2+3+1),(N+1))]
#define W_f_d1(c,z,y,x) array2[(c)]
...
#define W_ave_f_d2(c,z,y,x) array0[offset4((c),(z)+3,(y),(x)+3,(N+2+3+1),(N+1),(N+2+3+1))]
#define F_bar_f_d2(c,z,y,x) array1[offset4((c),(z)+3,(y),(x)+3,(N+2+3+1),(N+1),(N+2+3+1))]
#define W_f_d2(c,z,y,x) array2[(c)]
#define F_ave_f_d2(c,z,y,x) array3[offset4((c),(z)+2,(y),(x)+2,(N+1+2+1),(N+1),(N+1+2+1))]
#define W_ave_f_d3(c,z,y,x) array0[offset4((c),(z),(y)+3,(x)+3,(N+1),(N+2+3+1),(N+2+3+1))]
#define F_bar_f_d3(c,z,y,x) array1[offset4((c),(z),(y)+3,(x)+3,(N+1),(N+2+3+1),(N+2+3+1))]
#define W_f_d3(c,z,y,x) array2[(c)]
...
inline double euler_step(const double* U, double* rhs) {
    ...
    double* __restrict array1 = (double*) malloc(1866240*sizeof(double));
    double* __restrict array2 = (double*) malloc(5*sizeof(double));
    double* __restrict array3 = (double*) malloc(1715000*sizeof(double));
    double* __restrict array0 = (double*) malloc(1715000*sizeof(double));
    double retval;
...
for(t2 = 0; t2 <= C-1; t2++) {
  #pragma omp parallel for schedule(auto) private(t4,t6,t8,t10,t12,t14)
        ...
        for(t10 = max(-3,8*t4); t10 <= min(N+2,8*t4+7); t10++) {
          for(t12 = max(8*t6,2*t6-1); t12 <= min(8*t6+7,N+2); t12++) {
            ...
            for(t14 = max(5*t8,-3); t14 <= min(8*t8+7,0); t14++) {
                ...
                #pragma omp simd
            for(t14 = max(-3,6*t8); t14 <= min(8*t8+7,0); t14++) {
                  s0(t2,t10,t12,t14);
                  s5(t2,t10-1,t12,t14);
                  s6(t2,t10-2,t12,t14);
                }
