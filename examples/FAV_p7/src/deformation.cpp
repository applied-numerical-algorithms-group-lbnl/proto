...
void deformation::increment(const DF& a_DF){
 ...
 void deformation::QR(vector<deform>& Q, vector<deform>& R, vector<double>& angleQ, vector<double>& eigenR, vector<double>& detF ){

	 double e1[2], e2[2], u1[2], u2[2],v1[2], v2[2];
	 deform Q_m, R_m, QQT;
	 double uv, eigenV, deter;
	 ...
	 for( int i = 0; i < F.size(); i++){
	  ...
	  //f( i == 0){
	   ...
	   // Determinant
	   deter = v1[0]*v2[1] - v2[0]*v1[1];
           eigenV = max(abs(v1_norm), max(abs(uv), abs(u2_norm)));
	   eigenR.push_back(eigenV);
	   angleQ.push_back( acos(Q_m.F[0][0]) );
           detF.push_back( deter);
	 }
