...
namespace MHD_divB_Cleaning {
	...
	{
		for (int i=0; i< NUMCOMPS; i++){
			a_B_f_mapped1D(i) = a_N_s_d_ave_f(0)*a_B_ave_f(i) + a_dot_pro_sum(i)/12.0;			
		}
	}
            ...
            bool a_callBCs)
	{
	    ...
	    for (int d = 0; d < DIM; d++)
	    {
			...
			for (int s = 0; s < DIM; s++){
				...
				Vector B_f_mapped1D = forall<double,NUMCOMPS>(B_f_mapped1D_calc,B_ave_f,N_s_d_ave_f,dot_pro_sum);		
			...
			a_Rhs += m_divergence[d](B_f_mapped);
	    }
