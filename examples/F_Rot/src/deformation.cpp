...
//   Vg *= -1;
   ...
   for(auto it = Bg.begin(); it != Bg.end(); it++){
     ...
     Ug(r) += 0.5*(r[1]*a_State.m_dx - 1.5);
     Vg(r) += -0.5*(r[0]*a_State.m_dx - 1.5);
   ...
   dudxg = m_derivative[0](Ug, 1.0/a_State.m_dx); dudyg = m_derivative[1](Ug, 1.0/a_State.m_dx);
   dvdyg = m_derivative[1](Vg, 1.0/a_State.m_dx); dvdxg = m_derivative[0](Vg, 1.0/a_State.m_dx);
...
//     dudx = 0; dudy = 0.5; dvdx = -0.5; dvdy = 0; 
//cout << "F12 " << a_DF.F2.at(0).F[0][1] << " F21 " << a_DF.F2.at(0).F[1][0] << endl;
         
//cout << "original F12 " << state.F.at(0).F[0][1] << " original F21 " << state.F.at(0).F[1][0] << endl;
      for(unsigned int i = 0; i < state.F.size(); i++){
 ...
 //     cout << "F11 " << a_DF.F2.at(0).F[0][0] << " F22 " << a_DF.F2.at(0).F[1][1] << endl;
  

//      cout << dvdx<< " " << state.F.at(0).F[0][1] << " " << a_DF.F2.at(0).F[0][1] << " "<< (dvdx*(state.F.at(0).F[0][1] + a_DF.F2.at(0).F[0][1])) << endl;
...
void deformation::increment(const DF& a_DF){
       ...
       //  cout << "increment " << a_DF.F2.size() << " " << F.size() << endl:
...
//         cout << Q_m.F[0][0] << " Q entry " << acos(Q_m.F[0][0]) << " angle" << endl;
