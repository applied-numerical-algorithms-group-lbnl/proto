...
int main(int argc, char* argv[])
{
  ...
  int nx = 8;
    ...
    for (int ref=0;ref < maxRef; ref++)
      {
        for (int blkTest = 0;blkTest < 6; blkTest++)
          { 
            ...
            Operator::cubedSphereGeometry(X,NT,J,radius,bx,h,blkTest);
            h5.writePatch(1./nx,X,
                          "mapping_nx_"+to_string(nx)+"_block_"+to_string(blkTest));
            //h5.writePatch(1./nx,J,
            //            "Jacobian_nx_"+to_string(nx)+"_block_"+to_string(blkTest));
            for (int dir = 0; dir < DIM;dir++)
              {
                //  h5.writePatch(1./nx,NT[dir],
                //            "cofactor_nx_"+to_string(nx)+"_face_"+
                //            to_string(dir)+"_block_"+to_string(blkTest));
              }
            ...
            cout << "block "<< blkTest << " norm of div(NT): " << divNTNorm[ref] << endl;
            //h5.writePatch(1./nx,divNT,
            //            "divNT_nx_"+to_string(nx)+"_block_"+to_string(blockIndex));
            ...
            if (testCase < 2)
              {
                ...
                h5.writePatch(1./nx,divF,"divF"+to_string(nx)+"_block_"+to_string(blkTest));
                auto divfexact = divFExact(divF.box(),X,waveNumber);
                //h5.writePatch(1./nx,divfexact,"divFExact"+to_string(nx));
                divF -= divfexact;
                h5.writePatch(1./nx,divF,"divFError"+to_string(nx)+"_block_"+to_string(blkTest));            
