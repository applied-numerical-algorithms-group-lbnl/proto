...
\code{BaseOp} contains the following data members:
\begin{itemize}
...
\ForEach{\textbf{patch} $i$}
	\State $r_i \gets R_i - L(\phi_i)$ \Comment{No refluxing in $L(\phi)$}
\EndFor
...
\State{$R_C \gets R_C + \frac{-k}{dx_C}*\delta F$}\Comment{e.g. \code{F.reflux(RC, -k/(refRatio*dx))}}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\State $r_C \gets \langle R - L(\phi)\rangle\mid \mathcal{C}(\Omega_f)$\Comment{cleanup after reflux}
\State \textbf{return} $absMax(r)$\Comment{output is usually unused}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\item \code{m\_phiCAMR} the next coarser AMR level from which we interpolate boundary conditions. (Unused if AMR is false)
\end{itemize}
...
\Else
	...
	\State {$coarseRhs(R_C, \phi_C, \phi, R)$}\Comment{$R_C = \langle R - L(\phi) \rangle + L(\phi_C)$}
...
\If {\code{level == 0}}
	\State {$relax(\phi, R, BOTTOM\_RELAX)$}
\Else
	\State {$copyTo(\phi_C^{AMR}\rightarrow \phi_{C,temp}^{AMR})$}\Comment{regridding through \code{copyTo} call}
...
\Else
    ...
    \State $r_C\gets r^{AMR}[level-1]$
	 ...
	 \EndIf
	 \State {$coarseResidual(r_C, \phi_C, \rho_C, \phi, R, \delta F)$}
	 \State {$R_C \gets r_C + L(\phi_C)$}	 
	 \State {$AMRVcycle(\phi^{AMR}, \rho^{AMR}, r^{AMR}, R_C)$}\Comment {Recursive call}
	 \State {$fineCorrection(\phi, \phi_C, \phi_{C0})$}
	 \State {$interpBoundary(\phi, \phi_C)$} \Comment {high order interpolation}
	 \State {$vcycle(\phi, \phi_C, R)$} \Comment {MG V-Cycle with BC interp}
	 
	 \State{$residual(r, \phi, R)$}\Comment{for convergence}
	 \State{$coarseResidual(r_C, \phi_C, \rho_C, \phi, R, \delta F$} \Comment{for convergence}
\EndIf
