...
\caption{Reflux}
\begin{algorithmic}[1]
\Procedure{reflux}{$R_C, \phi_C, \phi, \delta F$}
\State {$\delta F \gets 0$} \Comment {Initialize}
\State{$exchange(\phi)$}
\State{$exchange(\phi_C)$}
\State{$\delta F \gets incrementFine(flux(\phi)) \mid \Omega_F$}
\State{$\delta F \gets incrementCoarse(flux(\phi_C)) \mid \Omega_C$}
\State{$R_C \gets R_C + \frac{-1}{dx_C}*\delta F$}\Comment{e.g. \code{F.reflux(RC, -1/(refRatio*dx))}}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\caption{computeRhs}
\begin{algorithmic}[1]
\Procedure{computeRhs (AMR Version)}{$R_C, \phi_C, \phi, R, \rho_C, \delta F$}
\State{$exchange(\phi)$}
\State{$exchange(\phi_C)$}
...
\begin{algorithmic}[1]
\Procedure{CoarseResidual}{$r_C, \phi, \phi_C, R_C, \delta F$}
\State $exchange(\phi_C)$
\State $r_C \gets -L(\phi_C)$
\State $reflux(r_C, \phi_C, \phi, \delta F)$
\State $r_C \gets r_C + R_C$
\State $r_C \gets 0 \mid \Omega_C^F$\Comment{cleanup after reflux}
\State \textbf{return} $absMax(r)$\Comment{output is usually unused}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\Else
    ...
    \State $r_C\gets r^{AMR}[level-1]$
	 ...
	 \State{$residual(r, \phi, R)$}\Comment{compute this level's residual}
	 \State{$coarseResidual(r_C, \phi, \phi_C, R_C, \delta F)$} \Comment{fix coarser level residual}
\EndIf
